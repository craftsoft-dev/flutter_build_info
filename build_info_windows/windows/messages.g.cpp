...
BuildInfoDataPigeon BuildInfoDataPigeon::FromEncodableList(const EncodableList& list) {
  ...
  if (!encodable_build_date.IsNull()) {
    decoded.set_build_date(encodable_build_date.LongValue());
  }
  auto& encodable_install_date = list[1];
  if (!encodable_install_date.IsNull()) {
    decoded.set_install_date(encodable_install_date.LongValue());
  }
  return decoded;
}


BuildInfoHostApiCodecSerializer::BuildInfoHostApiCodecSerializer() {}

EncodableValue BuildInfoHostApiCodecSerializer::ReadValueOfType(
  uint8_t type,
...
void BuildInfoHostApiCodecSerializer::WriteValue(
...
const flutter::StandardMessageCodec& BuildInfoHostApi::GetCodec() {
  return flutter::StandardMessageCodec::GetInstance(&BuildInfoHostApiCodecSerializer::GetInstance());
}
...
void BuildInfoHostApi::SetUp(
  ...
  {
    auto channel = std::make_unique<BasicMessageChannel<>>(binary_messenger, "dev.flutter.pigeon.build_info_windows.BuildInfoHostApi.fromPlatform", &GetCodec());
    if (api != nullptr) {
